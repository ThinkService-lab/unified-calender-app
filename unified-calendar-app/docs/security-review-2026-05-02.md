...
1. ~~Add a CI step that runs `npm audit --audit-level=high --production` on every PR and blocks on regressions (the `--production` flag excludes devDependencies — which is where today's 11 moderate advisories live).~~ **Closed 2026-05-03.** `.github/workflows/npm-audit.yml` runs `npm run audit:runtime` (`npm audit --audit-level=high --omit=dev`) and `npm run audit:full` (`npm audit --audit-level=high`) on every push to `main`, every PR, and on a weekly `cron: '0 13 * * 1'` schedule. A weekly informational step also uploads the full `npm audit --json` at moderate level as a build artifact (90-day retention) so future reviews can diff against the `docs/npm-audit-2026-05-02.json` baseline. The npm-script shape (audit-level, `--omit=dev` for the runtime gate, full-tree coverage for the second gate, moderate-level JSON for the weekly report, plus the workflow's presence on push/PR/schedule) is pinned by `src/__tests__/auditScripts.test.ts` so a later refactor cannot silently loosen the threshold.
...
4. ~~**M7 / weekly `npm audit` cadence + CI enforcement.**~~ **Closed 2026-05-03.** See recommendation #1 update above.

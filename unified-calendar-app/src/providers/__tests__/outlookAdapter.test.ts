/**
 * Unit tests for OutlookCalendarAdapter.
...
function createAdapter(mockAxios: ReturnType<typeof createMockAxiosInstance>) {
  ...
  const config: OutlookAdapterConfig = {
...
describe('OutlookCalendarAdapter', () => {
  ...
  it('should have providerId "outlook"', () => {
    expect(adapter.providerId).toBe('outlook');
  });
  ...
  describe('getChanges', () => {
    it('should perform initial sync without syncToken (deltaLink)', async () => {
      mockAxios.get.mockResolvedValueOnce({
        data: {
          value: [
  ...
  describe('executeBatch', () => {
    it('should send JSON batch request and parse responses', async () => {
      mockAxios.post.mockResolvedValueOnce({
        data: {
          responses: [
            { id: '1', status: 200, body: { id: 'evt-1', subject: 'Created' } },
            { id: '2', status: 200, body: { id: 'evt-2', subject: 'Updated' } },
          ],
        },
      });

      const requests: OutlookBatchRequest[] = [
        { method: 'POST', url: '/me/calendars/cal-1/events', body: { subject: 'New' } },
        { method: 'PATCH', url: '/me/calendars/cal-1/events/evt-2', body: { subject: 'Updated' } },
      ];

      const results = await adapter.executeBatch(requests);

      expect(results).toHaveLength(2);
      expect(results[0]).toEqual({ id: 'evt-1', subject: 'Created' });
      expect(results[1]).toEqual({ id: 'evt-2', subject: 'Updated' });
    });
    ...
    it('should return null for failed individual batch responses', async () => {
      ...
      const requests: OutlookBatchRequest[] = [
        { method: 'GET', url: '/me/calendars/cal-1/events/evt-1' },
        { method: 'GET', url: '/me/calendars/cal-1/events/evt-missing' },
      ];
    ...
    it('should throw if batch exceeds max size of 20', async () => {
      const requests: OutlookBatchRequest[] = Array.from({ length: 21 }, (_, i) => ({
        method: 'GET' as const,
        url: `/me/calendars/cal-1/events/evt-${i}`,
      }));

      await expect(adapter.executeBatch(requests)).rejects.toThrow(
        'Batch size 21 exceeds maximum of 20',
      );
    });
  });
});
...
describe('OutlookRateLimiter', () => {
  let limiter: OutlookRateLimiter;

  beforeEach(() => {
    limiter = new OutlookRateLimiter();
  });
